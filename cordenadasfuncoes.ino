...
//Calcilo de HoraAngulo em RA

double HAtoRA (double HSL, double HA)
{
  double RA;
  RA =  HSL - HA;
  while (RA < 0.0)
...
double RAtoHA (double HSL, double RA)
{
  double HA;
  HA = HSL - RA;
  while (HA < 0.0)
...
double JulianoDay (double y, double m, double d, double decH) {
  double JD = ( (367 * y) - floor((7 * (y + floor( (m + 9) / 12))) / 4) + floor(275 * m / 9) + (d - 730531.5) + (decH / 24));
  return JD;
